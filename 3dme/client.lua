...
RegisterCommand('sele', function(source, args)
    local text = '' -- edit here if you want to change the language : EN: the person / FR: la personne
    for i = 1,#args do
        --text = text .. 'Giver sele på personen' .. args[i]
    end
    text = text .. 'Giver sele på personen'
    TriggerServerEvent('3dme:shareDisplay', text)
end)

RegisterCommand('seleaf', function(source, args)
    local text = '' -- edit here if you want to change the language : EN: the person / FR: la personne
    for i = 1,#args do
        --text = text .. 'Tager selen af personen' .. args[i]
    end
    text = text .. 'Tager selen af personen'
    TriggerServerEvent('3dme:shareDisplay', text)
end)
